...
| `docs/development/ENGINEERING_PLATFORM_EXTRACTION_AUDIT.md` | C | Extraction-readiness evidence for the former DJConnect location. After `STANDALONE_EP_VERIFIED`, it informs the DJConnect-governed zero-live-duplicate audit required for `EP_EXTRACTION_CUTOVER_COMPLETE`; it does not authorize deletion in this repository. |
| `docs/development/ENGINEERING_PLATFORM_EXTRACTION_MIGRATION_PLAN.md` | B | Supplies the clean-slate/forensic-retention input for `MVP-MIG-001`: legacy-to-standalone uses a fresh official Schema 41 CENTRAL database and no legacy DB migration. EP owns the executable transition contract; B8C first qualifies DJConnect plus the EP source checkout as two development projects, B9 establishes `STANDALONE_EP_VERIFIED` through DJConnect, and only then may DJConnect conduct its separate extraction cutover. A later standalone-to-standalone CENTRAL relocation and EP self-hosting are separate EP-owned post-verification capabilities. Historical increment status remains evidence. |
...
- The active-semantics cleanup is deliberately later: after `STANDALONE_EP_VERIFIED`, DJConnect must remove or retire generic live EP implementation/runtime/authority duplicates and prove `EP_EXTRACTION_CUTOVER_COMPLETE` by a zero-live-duplicate audit. It preserves `.engineering-platform/repository.json`, required DJConnect-specific installed-EP consumer adapters, immutable prompts/receipts/provenance, the `HISTORY_ONLY` legacy store under preservation policy, and extraction evidence.
