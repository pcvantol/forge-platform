...
| W3 `MVP-REC-001` Persistence, backup, and recovery | EP and product storage contracts; Forge Platform qualifies deployment composition | **Current:** clean-store and transactional-recovery principles exist in historical EP evidence. **Target:** durable supported-store lifecycle, backup/restore and upgrade recovery for the installed composition, with one writable authority. | `MVP-EXEC-001`; product storage migration contract. | Restart/interruption, backup/restore, upgrade/rollback-or-fail-safe evidence; forensic legacy-store disposition. | Yes |
| W4 `MVP-INST-001` macOS installation lifecycle | Forge Platform coordinates product-owned provisioners | **Current:** strict source-level installer/catalog/composition schemas, self-update/preflight/provider/diff policy kernel, and native macOS wizard shell exist. After mandatory current-installer enforcement, the native core retains an exact signed catalog-feed locator and admits only an internal read-only outer catalog when strict bounded JSON, canonical Ed25519 threshold signatures, channel/freshness, exact release-trust-configuration scope and caller-supplied anti-replay evidence all agree. It neither transports nor persists the catalog, selects an index/manifest, creates a session, or changes a product; the default session preparer remains unavailable. No signed/notarized installer release, code-signed catalog-trust resource/transport, privileged bootstrapper, concrete product provisioner adapter, or installed-host qualification exists yet. **Target:** clean-machine macOS EP-only, Forge+EP, Workspace+EP, all-server, remote-peer, Developer Workstation and Custom flows; verified artifacts, product-owned system-domain `LaunchDaemon` registration through each product provisioner where required, discovery-driven topology bootstrap, authenticated product-API pairing/binding, receipts, upgrade, repair and uninstall. For EP, its provisioner retains installation/service/migration/cleanup authority; Forge Platform retains only coordination/readback evidence and is not a runtime dependency after install. | W0–W3; `MVP-COMP-001`; ADR-0006/instance contract. | Clean-install and repeatability tests; binding no-silent-retarget negatives; upgrade/repair/uninstall tests; least-privilege/security review; receipts and diagnostics. | Yes |
